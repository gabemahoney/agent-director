...
func Require() {
	if os.Getenv(EnvVar) == "" {
		fmt.Fprintln(os.Stderr,
			"refusing to run: the "+EnvVar+" marker is absent, so this process is running OUTSIDE the sandbox container (on the host), where it can rewrite the real ~/.agent-director.")
		fmt.Fprintln(os.Stderr,
			"Run it via `make test-sandbox` (see the run-tests skill). A broken-quoting `make sandbox CMD=\"…\"` escape is one way to end up on the host here.")
		os.Exit(1)
	}
}
