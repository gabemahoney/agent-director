...
- **CLI/MCP parity.** Every CLI verb must have an MCP-tool equivalent, and both must go through the shared `pkg/api` client — no capability may exist on one surface only, and no surface may bypass `pkg/api` to reach the store directly. New flags/parameters count: adding a flag to a verb means adding the matching parameter to the MCP tool *in the same change*. Agents driving agent-director over MCP must never be less capable than a human on the CLI. This parity is only partly structural: the MCP tool list (`internal/mcp/server.go` iterates `pkg/api/manifest.Verbs`) and both reference docs (`docs/cli-reference.md` and `docs/mcp-reference.md`, generated by `tools/gen-docs` from `pkg/api/manifest.Verbs`) come from that single verb list, but the CLI does not — its verbs are hand-wired in the `handlers()` map in `cmd/agent-director/main.go` with hand-written per-verb flag parsers (`spawn_cmd.go`, `flags.go`), and nothing under `cmd/agent-director` imports `pkg/api/manifest`. So CLI-side parity is a review-time obligation, not structurally guaranteed: when you add or change a verb or flag, you must hand-update the CLI handler to match the manifest. **Deliberate exceptions** (the only verbs that are CLI-only, filtered out of the MCP tool list by `internal/mcp.ExposedVerb`): `hook` (internal — invoked only by Claude Code's lifecycle-hook machinery, reads a payload from stdin), `serve` (starts the MCP server itself — exposing it as a tool would be self-referential), and `trail-emit` (a CLI-only write path that runs without opening `state.db` for corrupted-state recovery, with no MCP dispatch handler). There are no other exceptions: every other verb maps 1:1 to an MCP tool with identical parameters. When you add a new verb or flag, either it lands on both surfaces or you add it to the `ExposedVerb` exception list *and* document why here.
