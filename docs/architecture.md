...
`agent-director trail-path` prints the resolved trail file path to stdout
(one line, no JSON envelope) and exits 0. It works even when `state.db` is
missing or corrupted — it is a recovery aid that bypasses the normal client
setup path.

Canonical invocations:

```sh
# Default — AGENT_DIRECTOR_STATE_DIR not set
$ agent-director trail-path
/home/alice/.agent-director/ad-trail.jsonl

# Custom state dir
$ AGENT_DIRECTOR_STATE_DIR=/var/lib/ad agent-director trail-path
/var/lib/ad/ad-trail.jsonl
```

The printed path reflects `AGENT_DIRECTOR_STATE_DIR` **at process invocation
time** — it is not cached or computed from the installation default.
Wrapping scripts that override the env var get the correct path without
post-processing.

`docs/cli-reference.md` is auto-regenerated by `make generate` (via the
`//go:generate` directive in `pkg/api/manifest/manifest.go`). The `trail-path`
verb appears there under `## trail-path` with its full parameter and error
tables.
...
raw file (SR-A-6.1, SR-A-7.14, SR-A-7.15).

Follow new events live:

```sh
tail -f ~/.agent-director/ad-trail.jsonl | jq .
```

All events for a specific request token:
